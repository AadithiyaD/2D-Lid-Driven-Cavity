...
reynolds_number = int((1.0 * u_bc ) / nu) # Re=Dv/nu, D=1.0, nu=0.1, v from bc setting


def load_field(name):
    values = np.loadtxt(DATA / f"{name}_Re_{reynolds_number}.csv", delimiter=",", skiprows=1)
...
# Load and plot the final velocity, pressure, and velocity magnitude fields.
...
for axis, name in zip(axes, ("u", "v", "p", "|U|")):
    field = {"u": u, "v": v, "p": p, "|U|": u_magnitude}[name]
...
u_timesteps, u_l2, u_infinity = load_residual_norms(f"u_residual_norms_Re_{reynolds_number}")
v_timesteps, v_l2, v_infinity = load_residual_norms(f"v_residual_norms_Re_{reynolds_number}")
